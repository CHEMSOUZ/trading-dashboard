...
const IMPACT_CONFIG = {
  ...
  low:    { label: 'FAIBLE', color: '#5a6a82', bg: 'rgba(136,153,187,0.05)',  border: 'rgba(136,153,187,0.10)',  dot: '#5a6a82' },
  na:     { label: '—',      color: '#3a1818', bg: 'rgba(14,15,22,0.3)',    border: 'rgba(136,153,187,0.05)',  dot: '#3a1818' },
};
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
      {['high','medium','low'].map((lvl, i) => {
        ...
        return <div key={lvl} style={{ width: '7px', height: '7px', borderRadius: '50%', background: lit ? col : 'rgba(136,153,187,0.12)' }} />;
      })}
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      background: isToday
        ...
        : 'rgba(0,0,0,0.2)',
      borderTop: '1px solid rgba(136,153,187,0.10)',
      borderBottom: `1px solid ${isToday ? 'rgba(136,153,187,0.14)' : 'rgba(136,153,187,0.05)'}`,
    }}>
      {/* Colored left accent */}
      <div style={{
        width: '3px', height: '28px', borderRadius: '2px',
        background: isToday ? '#8899bb' : isTomorrow ? '#f0a020' : '#1a4a2a',
        flexShrink: 0,
      }} />
      <div>
        <div style={{
          fontSize: '13px', fontWeight: '700', letterSpacing: '2px',
          color: isToday ? '#8899bb' : isTomorrow ? '#f0a020' : '#6a9a7a',
        }}>
          {label}
        </div>
        {relLabel && (
      ...
      <div
        ...
        style={{
          ...
          background: upcoming30
            ...
            : hasActual ? 'rgba(14,15,22,0.5)'
            : past ? 'rgba(14,15,22,0.2)'
            : 'rgba(14,15,22,0.38)',
        ...
        onMouseLeave={e => e.currentTarget.style.background = upcoming30 ? 'rgba(240,160,32,0.07)' : hasActual ? 'rgba(14,15,22,0.5)' : past ? 'rgba(14,15,22,0.2)' : 'rgba(14,15,22,0.38)'}
      >
        {/* Heure + fuseau */}
        ...
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#3c4c64" strokeWidth="2" strokeLinecap="round">
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 'none', fontFamily: "'JetBrains Mono','Fira Code',monospace" }}>
      <style>{`
        ...
        ::-webkit-scrollbar-thumb { background: rgba(136,153,187,0.22); border-radius: 2px; }
      `}</style>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          {lastUpdate && (
            <div style={{ fontSize: '10px', color: '#3c4c64', textAlign: 'right' }}>
              ...
              <div style={{ color: '#1e2c40', marginTop: '2px' }}>Fuseau : {TIMEZONES.find(z => z.key === tz)?.flag} {tzAbbr} · {TIMEZONES.find(z => z.key === tz)?.label}</div>
            </div>
          )}
      ...
      {/* ── Filters ── */}
      <div style={{ background: 'rgba(14,15,22,0.4)', border: '1px solid rgba(136,153,187,0.10)', borderRadius: '8px', padding: '14px 16px', marginBottom: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
      ...
      {!loading && filtered.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(160px,1fr))', gap: '8px', marginBottom: '14px' }}>
          {[
            { label: 'TOTAL',       value: filtered.length, color: '#dde4ef' },
            { label: 'FORT IMPACT', value: highCount,        color: '#ff4455' },
            { label: 'PUBLIÉS',     value: withActual,       color: '#8899bb' },
      ...
      {loading ? (
        ...
        <div style={{ background: 'rgba(14,15,22,0.3)', border: '1px solid rgba(136,153,187,0.08)', borderRadius: '8px', overflow: 'hidden' }}>
