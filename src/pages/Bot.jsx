  ...
  return (
    <div style={{ background: bgColor, border: `1px solid ${accentColor}25`, borderLeft: `3px solid ${accentColor}`, borderRadius: '8px', padding: '18px 20px', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
        ...
        {signal.bot && (() => { const b = PINE_BOTS.find(x => x.botId === signal.bot); const c = b?.color ?? '#aa88ff'; return <div style={{ fontSize:'12px', color: c, background: `${c}15`, border: `1px solid ${c}40`, padding: '2px 8px', borderRadius: '3px', fontWeight: '700', letterSpacing: '1px' }}>{signal.bot}</div>; })()}
        {fmtTf(signal.timeframe) && <div style={{ fontSize:'13px', color: '#5a6a82', background: 'rgba(136,153,187,0.06)', border: '1px solid rgba(136,153,187,0.12)', padding: '2px 7px', borderRadius: '3px' }}>{fmtTf(signal.timeframe)}</div>}
        <div style={{ marginLeft: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '1px' }}>
          ...
          {timeBougie && timeBougie !== timeRecu && (
            <div style={{ fontSize:'12px', color: '#5a6a82' }}>bougie : {timeBougie}</div>
          )}
          <div style={{ fontSize:'12px', color: '#3a1818' }}>heure Paris</div>
        </div>
        {signal.type === 'test' && <div style={{ fontSize:'11px', background: 'rgba(240,120,32,0.15)', border: '1px solid rgba(240,120,32,0.4)', color: '#f07820', padding: '2px 6px', borderRadius: '3px', letterSpacing: '1px', fontWeight: '700' }}>TEST</div>}
        {isLatest && <div style={{ fontSize:'11px', background: 'rgba(136,153,187,0.18)', border: '1px solid rgba(136,153,187,0.35)', color: '#8899bb', padding: '2px 6px', borderRadius: '3px', letterSpacing: '1px', fontWeight: '700' }}>NOUVEAU</div>}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '14px' }}>
        ...
        <div style={{ background: 'rgba(14,15,22,0.6)', border: '1px solid rgba(136,153,187,0.18)', borderRadius: '6px', padding: '10px 14px' }}>
          <div style={{ fontSize:'12px', color: '#5a6a82', letterSpacing: '2px', marginBottom: '4px' }}>📍 ENTRÉE</div>
        ...
        <div style={{ background: 'rgba(255,68,85,0.05)', border: '1px solid rgba(255,68,85,0.15)', borderRadius: '6px', padding: '10px 14px' }}>
          <div style={{ fontSize:'12px', color: '#4a5a72', letterSpacing: '2px', marginBottom: '4px' }}>🛑 STOP LOSS</div>
          <div style={{ fontSize: '18px', fontWeight: '700', color: '#ff7788' }}>{sl.toFixed(2)}</div>
          <div style={{ fontSize:'12px', color: '#5a3a3a', marginTop: '2px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
        ...
        {signal.context && (
          <div style={{ fontSize:'13px', color: '#6878a0', fontStyle: 'italic', flex: 1 }}>{signal.context}</div>
        )}
      </div>
      ...
      {onSave && (
        <button onClick={() => onSave(signal)}
          style={{ width: '100%', padding: '9px', background: 'transparent', border: `1px dashed ${accentColor}50`, borderRadius: '5px', color: accentColor, fontSize:'13px', fontFamily: 'inherit', letterSpacing: '2px', cursor: 'pointer', transition: 'all 0.15s' }}
...
function BSStatCard({ label, value, sub, color = '#dde4ef' }) {
  return (
    <div style={{ background: 'rgba(14,15,22,0.5)', border: '1px solid rgba(136,153,187,0.10)', borderTop: `2px solid ${color}`, borderRadius: '6px', padding: '14px 16px' }}>
      <div style={{ fontSize:'12px', color: '#5a6a82', letterSpacing: '2px', marginBottom: '6px' }}>{label}</div>
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize:'12px', color: '#5a6a82', letterSpacing: '1px', marginBottom: '3px' }}>{title}</div>
        <div style={{ fontSize: '15px', fontWeight: '700', color, marginBottom: '2px' }}>{value}</div>
        <div style={{ fontSize:'13px', color: '#5868a0' }}>{desc}</div>
      </div>
      {onClick && <div style={{ fontSize:'12px', color: active ? color : '#3a1818', letterSpacing: '1px' }}>{active ? '▲' : '▼'}</div>}
    </div>
  );
}
...
function BotStats({ signals }) {
  ...
  if (rated.length === 0) return (
    <div style={{ textAlign: 'center', padding: '60px 20px', border: '1px dashed rgba(136,153,187,0.12)', borderRadius: '8px' }}>
      ...
      <div style={{ fontSize:'13px', color: '#3a1818' }}>Utilise les boutons W / L / BE dans l'onglet SIGNAUX pour noter chaque position</div>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        <span style={{ fontSize:'13px', color: '#8899bb', letterSpacing: '2px', fontWeight: '700', whiteSpace: 'nowrap' }}>✅ POINTS FORTS — cliquer pour voir les positions</span>
        <div style={{ height: '1px', flex: 1, background: 'rgba(136,153,187,0.12)' }} />
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        <span style={{ fontSize:'13px', color: '#ff4455', letterSpacing: '2px', fontWeight: '700', whiteSpace: 'nowrap' }}>❌ POINTS FAIBLES — cliquer pour voir les positions</span>
        <div style={{ height: '1px', flex: 1, background: 'rgba(255,68,85,0.1)' }} />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: '10px' }}>
      ...
      {drill && drillSignals.length > 0 && (
        <div style={{ background: 'rgba(8,9,16,0.6)', border: `1px solid ${drill.color}30`, borderRadius: '8px', padding: '16px 18px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
            <span style={{ fontSize:'13px', color: drill.color, letterSpacing: '2px', fontWeight: '700' }}>{drill.label}</span>
            <span style={{ fontSize:'12px', background: `${drill.color}15`, border: `1px solid ${drill.color}30`, color: drill.color, padding: '1px 8px', borderRadius: '10px' }}>{drillSignals.length} trades</span>
            <span style={{ fontSize:'12px', color: '#8899bb', fontWeight: '700' }}>✓ {drillSignals.filter(s => s._outcome === 'win').length}W</span>
            <span style={{ fontSize:'12px', color: '#ff4455', fontWeight: '700' }}>✗ {drillSignals.filter(s => s._outcome === 'loss').length}L</span>
            {drillSignals.filter(s => s._outcome === 'be').length > 0 && <span style={{ fontSize:'12px', color: '#f0c020', fontWeight: '700' }}>— {drillSignals.filter(s => s._outcome === 'be').length}BE</span>}
            <button onClick={() => setDrill(null)} style={{ marginLeft: 'auto', background: 'none', border: '1px solid rgba(255,68,85,0.2)', borderRadius: '3px', color: '#5a3a3a', fontSize:'12px', fontFamily: 'inherit', cursor: 'pointer', padding: '2px 8px' }}
          ...
          {['win', 'loss', 'be'].map(oc => {
            ...
            return (
              <div key={oc} style={{ marginBottom: '10px' }}>
                <div style={{ fontSize:'12px', color: ocColor, letterSpacing: '2px', fontWeight: '700', marginBottom: '6px' }}>{ocLabel} ({ocSigs.length})</div>
                  ...
                  {ocSigs.map((sig, i) => {
                    ...
                    return (
                      <div key={sig._id ?? i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '5px 10px', background: 'rgba(14,15,22,0.5)', borderRadius: '4px', borderLeft: `2px solid ${ocColor}60` }}>
                        <span style={{ fontSize:'12px', color: '#5868a0', minWidth: '70px' }}>{fmtTime(sig._receivedAt)}</span>
                        {tf && <span style={{ fontSize:'12px', color: '#3a7a5a', background: 'rgba(136,153,187,0.08)', border: '1px solid rgba(136,153,187,0.12)', padding: '0px 4px', borderRadius: '2px', fontWeight: '700' }}>{tf}</span>}
                        <span style={{ fontSize:'12px', color: isL ? '#8899bb' : '#ff4455', fontWeight: '700' }}>{isL ? '▲' : '▼'} {dir}</span>
                        {sig.bot && <span style={{ fontSize:'12px', color: '#7a5a9a' }}>{sig.bot}</span>}
                        <span style={{ fontSize:'12px', color: '#6a8a7a' }}>{parseFloat(sig.entry)?.toFixed(2) ?? '—'}</span>
                        <span style={{ fontSize:'12px', color: pnlColor(pnl), fontWeight: '700', marginLeft: 'auto' }}>{fmtUsd(pnl, true)}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {byBot.map(b => (
                <div key={b.label} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '6px 10px', background: 'rgba(14,15,22,0.4)', borderRadius: '4px', borderLeft: `2px solid ${pnlColor(b.pnl)}` }}>
                  <span style={{ fontSize:'13px', color: '#dde4ef', flex: 1 }}>{b.label}</span>
                  <span style={{ fontSize:'13px', fontWeight: '700', color: pnlColor(b.pnl) }}>{fmtUsd(b.pnl, true)}</span>
                  <span style={{ fontSize:'13px', color: b.wr >= 50 ? '#00cc77' : '#ff3344' }}>{b.wr}% WR</span>
                  <span style={{ fontSize:'13px', color: '#5a6a82' }}>{b.total}T</span>
                </div>
              ))}
            </div>
          </BSSection>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            {byDir.filter(d => d.total > 0).map(d => (
              <div key={d.label} style={{ background: 'rgba(14,15,22,0.4)', borderRadius: '6px', padding: '10px 14px', borderLeft: `3px solid ${d.label === 'LONG' ? '#00cc77' : '#ff3344'}` }}>
                ...
                <div style={{ fontSize:'13px', color: '#5868a0', marginTop: '3px' }}>{d.wr}% WR · {d.wins}W {d.total - d.wins}L</div>
              </div>
            ))}
          </div>
        </BSSection>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            {byKZ.map(k => (
              <div key={k.label} style={{ background: 'rgba(14,15,22,0.5)', border: '1px solid rgba(136,153,187,0.08)', borderRadius: '5px', padding: '6px 10px', textAlign: 'center' }}>
                <div style={{ fontSize:'13px', color: '#5868a0', marginBottom: '2px' }}>{k.label}</div>
                <div style={{ fontSize:'13px', fontWeight: '700', color: pnlColor(k.pnl) }}>{fmtUsd(k.pnl, true)}</div>
                <div style={{ fontSize:'12px', color: k.wr >= 50 ? '#00cc77' : '#ff3344' }}>{k.wr}% WR · {k.total}T</div>
              </div>
            ))}
          </div>
        </BSSection>
            ...
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
              {byDay.map(d => (
                <div key={d.label} style={{ background: 'rgba(14,15,22,0.5)', border: '1px solid rgba(136,153,187,0.08)', borderRadius: '5px', padding: '5px 8px', textAlign: 'center' }}>
                  <div style={{ fontSize:'13px', color: '#5868a0', textTransform: 'capitalize' }}>{d.label}</div>
                  <div style={{ fontSize:'13px', fontWeight: '700', color: pnlColor(d.pnl) }}>{fmtUsd(d.pnl, true)}</div>
                  <div style={{ fontSize:'12px', color: d.wr >= 50 ? '#00cc77' : '#ff3344' }}>{d.wr}% · {d.total}T</div>
                </div>
              ))}
            </div>
          </BSSection>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
          {['ALL', 'WIN', 'LOSS', 'BE'].map(f => {
            ...
            return (
              <button key={f} onClick={() => setFilter(f)}
                style={{ padding: '4px 12px', borderRadius: '4px', border: `1px solid ${filter === f ? c : '#1e2c40'}`, background: filter === f ? `${c}18` : 'transparent', color: filter === f ? c : '#5a6a82', fontSize:'13px', fontFamily: 'inherit', cursor: 'pointer' }}>
                {f} ({count})
              </button>
            );
          })}
          {botList.length > 2 && botList.map(b => (
            <button key={b} onClick={() => setBotFilter(b)}
              style={{ padding: '4px 10px', borderRadius: '4px', border: `1px solid ${botFilter === b ? '#aa88ff' : 'rgba(136,153,187,0.10)'}`, background: botFilter === b ? 'rgba(170,136,255,0.12)' : 'transparent', color: botFilter === b ? '#aa88ff' : '#3a5a3a', fontSize:'12px', fontFamily: 'inherit', cursor: 'pointer' }}>
              {b}
            </button>
          ))}
        </div>
            ...
            <thead style={{ position: 'sticky', top: 0, zIndex: 1 }}>
              <tr style={{ background: 'rgba(6,12,8,0.95)', borderBottom: '1px solid rgba(136,153,187,0.10)' }}>
                {['HEURE', 'TF', 'BOT', 'SCRIPT', 'DIR', 'ENTRY', 'BE', 'SL', 'TP', 'R:R', 'PTS', 'P&L (5 MNQ)', 'SESSION KZ', 'RÉSULTAT'].map(h => (
                  <th key={h} style={{ padding: '8px 10px', textAlign: 'left', fontSize:'12px', color: '#5a6a82', letterSpacing: '1.5px', fontWeight: '700', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {listSignals.length === 0 && (
                <tr><td colSpan={14} style={{ padding: '24px', textAlign: 'center', color: '#3a1818', fontSize:'13px' }}>Aucun signal</td></tr>
              )}
              {listSignals.map((sig, i) => {
                ...
                return (
                  ...
                  >
                    <td style={{ padding: '7px 10px', color: '#5868a0', fontSize:'13px', whiteSpace: 'nowrap' }}>{fmtTime(sig._receivedAt)}</td>
                    <td style={{ padding: '7px 6px' }}>
                      {fmtTf(sig.timeframe) ? <span style={{ fontSize:'12px', color: '#3a9a5a', background: 'rgba(136,153,187,0.08)', border: '1px solid rgba(136,153,187,0.12)', padding: '1px 4px', borderRadius: '2px', fontWeight: '700' }}>{fmtTf(sig.timeframe)}</span> : <span style={{ color: '#3a1818' }}>—</span>}
                    </td>
                    <td style={{ padding: '7px 10px', fontSize:'12px', color: '#aa88ff' }}>{sig.bot ?? '—'}</td>
                    <td style={{ padding: '7px 6px' }}>
                      {(() => { const s = getBotScriptName(sig.bot); return s ? <span style={{ fontSize:'12px', color: s.color, background: `${s.color}18`, border: `1px solid ${s.color}40`, padding: '2px 6px', borderRadius: '3px', fontWeight: '700', letterSpacing: '0.5px', whiteSpace: 'nowrap' }}>{s.name}</span> : <span style={{ color: '#3a1818' }}>—</span>; })()}
                    </td>
                    <td style={{ padding: '7px 10px' }}>
                      <span style={{ color: isL ? '#8899bb' : '#ff4455', background: `rgba(${isL ? '0,204,119' : '255,51,68'},0.08)`, padding: '1px 5px', borderRadius: '3px', fontSize:'13px', fontWeight: '600' }}>{dir}</span>
                    </td>
                    ...
                    <td style={{ padding: '7px 10px', fontSize:'12px', color: '#5a6a82', whiteSpace: 'nowrap' }}>{kzLabel}</td>
                    <td style={{ padding: '7px 10px' }}>
                      <span style={{ fontSize:'12px', fontWeight: '700', color: ocC, background: `${ocC}15`, border: `1px solid ${ocC}40`, padding: '2px 7px', borderRadius: '3px' }}>{ocTxt}</span>
                    </td>
                  </tr>
                );
              })}
...
function CustomTVInput() {
  ...
  const inp = { background: 'rgba(14,15,22,0.6)', border: '1px solid rgba(136,153,187,0.18)', borderRadius: '5px', padding: '8px 11px', color: '#dde4ef', fontSize:'13px', fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' };
  function open() {
  ...
  return (
    <>
      ...
      <button onClick={open} style={{ padding: '8px 16px', background: 'transparent', border: '1px solid rgba(136,153,187,0.35)', borderRadius: '5px', color: '#8899bb', fontSize:'13px', fontFamily: 'inherit', cursor: 'pointer', letterSpacing: '1px', whiteSpace: 'nowrap', transition: 'all 0.15s' }}
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: '960px' }} ref={flashRef}>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
          <div>
            ...
            <div style={{ fontSize:'13px', color: '#5a6a82', letterSpacing: '2px' }}>MNQ · SIGNAUX ICT / SMC EN TEMPS RÉEL</div>
          </div>
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontSize:'13px', color: serverOk ? '#566880' : '#ff4455', letterSpacing: '1px' }}>{serverOk ? 'SERVEUR ACTIF' : 'SERVEUR ERREUR'}</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(0,255,136,0.03)', border: '1px solid rgba(136,153,187,0.14)', borderRadius: '6px', padding: '10px 14px', marginBottom: '20px' }}>
        <span style={{ fontSize:'12px', color: '#5a6a82', letterSpacing: '2px', flexShrink: 0 }}>WEBHOOK URL</span>
        <code style={{ flex: 1, fontSize:'13px', color: '#8899bb', background: 'rgba(136,153,187,0.08)', padding: '4px 10px', borderRadius: '4px', letterSpacing: '0.5px' }}>
          {webhookUrl}
        </code>
        <button onClick={() => copyText(webhookUrl, 'url')}
          style={{ padding: '5px 12px', background: copied === 'url' ? 'rgba(136,153,187,0.22)' : 'transparent', border: '1px solid rgba(136,153,187,0.35)', borderRadius: '4px', color: '#8899bb', fontSize:'13px', fontFamily: 'inherit', cursor: 'pointer', transition: 'all 0.15s', letterSpacing: '1px' }}>
          {copied === 'url' ? '✓ COPIÉ' : 'COPIER'}
        </button>
        <div style={{ display: 'flex', alignItems: 'center', gap: '5px', borderLeft: '1px solid rgba(136,153,187,0.10)', paddingLeft: '10px' }}>
          <span style={{ fontSize:'12px', color: '#5a6a82' }}>PORT</span>
          <input value={portInput} onChange={e => setPortInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSetPort()} style={{ ...inp, width: '60px' }} />
          <button onClick={handleSetPort} style={{ padding: '5px 8px', background: 'transparent', border: '1px solid rgba(136,153,187,0.22)', borderRadius: '4px', color: '#5a6a82', fontSize:'13px', fontFamily: 'inherit', cursor: 'pointer' }}>OK</button>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '4px', marginBottom: '20px', borderBottom: '1px solid rgba(136,153,187,0.10)', paddingBottom: '0' }}>
        {[
          ...
          <button key={t.key} onClick={() => setTab(t.key)}
            style={{ padding: '8px 16px', background: 'transparent', border: 'none', borderBottom: `2px solid ${tab === t.key ? '#8899bb' : 'transparent'}`, color: tab === t.key ? '#8899bb' : '#5a6a82', fontSize:'13px', fontFamily: 'inherit', letterSpacing: '2px', cursor: 'pointer', transition: 'all 0.15s', marginBottom: '-1px' }}>
            {t.label}
          </button>
        ))}
        {signals.length > 0 && (
          <button onClick={handleClearSignals} style={{ marginLeft: 'auto', padding: '6px 12px', background: 'transparent', border: '1px solid rgba(255,68,85,0.15)', borderRadius: '4px', color: '#5a3a3a', fontSize:'12px', fontFamily: 'inherit', cursor: 'pointer', letterSpacing: '1px', transition: 'all 0.15s', marginBottom: '2px' }}
            onMouseEnter={e => { e.currentTarget.style.color = '#ff4455'; e.currentTarget.style.borderColor = 'rgba(255,68,85,0.4)'; }}
      ...
      {stats && stats.total > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '10px 16px', background: 'rgba(0,255,136,0.03)', border: '1px solid rgba(136,153,187,0.10)', borderRadius: '6px', marginBottom: '16px', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            <span style={{ fontSize:'12px', color: '#5a6a82', letterSpacing: '2px' }}>HISTORIQUE SAUVEGARDÉ</span>
          </div>
          <div style={{ display: 'flex', gap: '14px', flex: 1, flexWrap: 'wrap' }}>
            <span style={{ fontSize:'13px', color: '#dde4ef', fontWeight: '700' }}>{stats.total} signal{stats.total > 1 ? 's' : ''}</span>
            <span style={{ fontSize:'13px', color: '#8899bb' }}>▲ {stats.longs} LONG</span>
            <span style={{ fontSize:'13px', color: '#ff4455' }}>▼ {stats.shorts} SHORT</span>
            {stats.oldest && (
              <span style={{ fontSize:'13px', color: '#5a6a82' }}>
          ...
          <div style={{ fontSize:'12px', color: '#3a1818', fontStyle: 'italic' }}>persisté au redémarrage</div>
        </div>
      )}
      ...
      {tab === 'signals' && (
        <div>
              ...
              {botIds.map(bid => {
                ...
                return (
                  <button key={bid} onClick={() => setSelectedBot(bid)}
                    style={{ padding: '5px 14px', borderRadius: '4px', border: `1px solid ${sel ? c + '80' : 'rgba(136,153,187,0.12)'}`, background: sel ? `${c}15` : 'transparent', color: sel ? c : '#5a6a82', fontSize:'13px', fontFamily: 'inherit', fontWeight: sel ? '700' : '400', cursor: 'pointer', transition: 'all 0.15s', letterSpacing: '1px', position: 'relative' }}>
                    {bid} <span style={{ opacity: 0.6 }}>({cnt})</span>
                    {activeCnt > 0 && (
                      <span style={{ position: 'absolute', top: '-4px', right: '-4px', background: '#f0c020', color: '#1a1a00', fontSize:'11px', fontWeight: '900', borderRadius: '8px', padding: '0px 4px', minWidth: '14px', textAlign: 'center', lineHeight: '14px', animation: 'pulse 2s infinite' }}>{activeCnt}</span>
                    )}
                  </button>
                );
              })}
          ...
          {completedSignals.length > 0 && (() => {
            ...
            const ptsColor = st.totalPts >= 0 ? '#00cc77' : '#ff3344';
            return (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(110px, 1fr))', gap: '8px', marginBottom: '16px' }}>
                {[
                  { label: 'WINRATE', value: st.winrate != null ? `${st.winrate}%` : '—', sub: `${st.wins}W · ${st.losses}L${st.bes > 0 ? ` · ${st.bes}BE` : ''}`, color: st.winrate != null && st.winrate >= 50 ? '#00cc77' : '#ff3344' },
                  ...
                  <div key={card.label} style={{ background: 'rgba(14,15,22,0.5)', border: `1px solid ${card.color}20`, borderRadius: '6px', padding: '10px 14px' }}>
                    <div style={{ fontSize:'12px', color: '#5a6a82', letterSpacing: '2px', marginBottom: '4px' }}>{card.label}</div>
                    <div style={{ fontSize: '16px', fontWeight: '700', color: card.color }}>{card.value}</div>
                    <div style={{ fontSize:'12px', color: '#3a5a3a', marginTop: '2px' }}>{card.sub}</div>
                  </div>
                ))}
              </div>
            );
          ...
          {saveMsg && (
            <div style={{ marginBottom: '14px', padding: '10px 14px', background: saveMsg.startsWith('Erreur') ? 'rgba(255,68,85,0.1)' : 'rgba(136,153,187,0.12)', border: `1px solid ${saveMsg.startsWith('Erreur') ? 'rgba(255,68,85,0.3)' : 'rgba(136,153,187,0.35)'}`, borderRadius: '5px', fontSize:'13px', color: saveMsg.startsWith('Erreur') ? '#ff4455' : '#8899bb' }}>
              {saveMsg}
            </div>
          )}
          ...
          {filtered.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px', border: '1px dashed rgba(136,153,187,0.12)', borderRadius: '8px' }}>
              ...
              <div style={{ fontSize:'13px', color: '#3a1818' }}>Configure ngrok + TradingView pour recevoir les alertes</div>
              <button onClick={() => setTab('setup')} style={{ marginTop: '14px', padding: '7px 16px', background: 'transparent', border: '1px solid rgba(136,153,187,0.22)', borderRadius: '4px', color: '#566880', fontSize:'13px', fontFamily: 'inherit', cursor: 'pointer', letterSpacing: '1px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              ...
              {activeSignals.length === 0 && latest && (
                <div>
                  <div style={{ fontSize:'12px', color: '#5a6a82', letterSpacing: '2px', marginBottom: '10px' }}>DERNIER SIGNAL</div>
              ...
              {activeSignals.length > 0 && (
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                    ...
                    <span style={{ fontSize:'12px', color: '#f0c020', letterSpacing: '2px', fontWeight: '700' }}>SLOTS EN COURS</span>
                    <span style={{ fontSize:'12px', background: 'rgba(240,192,32,0.12)', border: '1px solid rgba(240,192,32,0.3)', color: '#f0c020', padding: '1px 8px', borderRadius: '10px', fontWeight: '700' }}>{activeSignals.length}</span>
                    <span style={{ fontSize:'12px', color: '#3a5a3a' }}>— BE manuel ou auto (retour entrée) libère le slot TradingView</span>
                  </div>
                      ...
                      return (
                        <div key={sig._id ?? i} style={{ background: `rgba(${dirRgb},0.04)`, border: `1px solid rgba(${dirRgb},0.15)`, borderLeft: `3px solid ${dirColor}`, borderRadius: '7px', padding: '12px 16px' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                            ...
                            {sig.bot && <span style={{ fontSize:'12px', color: botColor, background: `${botColor}15`, border: `1px solid ${botColor}40`, padding: '1px 6px', borderRadius: '3px', fontWeight: '700' }}>{sig.bot}</span>}
                            {tf && <span style={{ fontSize:'12px', color: '#b09020', background: 'rgba(240,192,32,0.1)', border: '1px solid rgba(240,192,32,0.25)', padding: '1px 5px', borderRadius: '3px', fontWeight: '700' }}>{tf}</span>}
                            {sig.type === 'test' && <span style={{ fontSize:'11px', background: 'rgba(240,120,32,0.15)', border: '1px solid rgba(240,120,32,0.4)', color: '#f07820', padding: '1px 4px', borderRadius: '2px', fontWeight: '700' }}>TEST</span>}
                            <span style={{ marginLeft: 'auto', fontSize:'12px', color: '#5868a0' }}>{fmtTime(sig._receivedAt)}</span>
                          </div>
                          {/* Niveaux */}
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: '6px', marginBottom: '10px' }}>
                            <div style={{ background: 'rgba(14,15,22,0.5)', borderRadius: '5px', padding: '7px 10px' }}>
                              <div style={{ fontSize:'12px', color: '#5a6a82', letterSpacing: '1px', marginBottom: '2px' }}>📍 ENTRÉE</div>
                            ...
                            <div style={{ background: 'rgba(255,68,85,0.06)', borderRadius: '5px', padding: '7px 10px' }}>
                              <div style={{ fontSize:'12px', color: '#4a5a72', letterSpacing: '1px', marginBottom: '2px' }}>🛑 SL {slPts && <span style={{ color: '#3a2a2a' }}>−{slPts}pts</span>}</div>
                          ...
                          <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                            <span style={{ fontSize:'12px', color: '#3a5a3a', marginRight: '2px' }}>Clôturer :</span>
                            {[['win','✓ WIN','#8899bb'],['loss','✗ LOSS','#ff4455'],['be','— BE','#f0c020']].map(([key, label, c]) => (
                              <button key={key} onClick={() => handleUpdateOutcome(sig._id, sig._outcome, key)}
                                style={{ padding: '4px 12px', borderRadius: '4px', border: `1px solid ${c}60`, background: `${c}15`, color: c, fontSize:'13px', fontFamily: 'inherit', fontWeight: '700', cursor: 'pointer', transition: 'all 0.12s', letterSpacing: '0.5px' }}
                            ...
                            <div style={{ marginLeft: 'auto', display: 'flex', gap: '5px' }}>
                              <button onClick={() => handleSaveSignal(sig)} title="Sauvegarder dans le journal"
                                style={{ padding: '4px 10px', background: 'none', border: '1px solid rgba(136,153,187,0.18)', borderRadius: '4px', color: '#5a6a82', fontSize:'12px', fontFamily: 'inherit', cursor: 'pointer', transition: 'all 0.12s' }}
                              ...
                              <button onClick={() => { if (window.confirm('Ignorer ce signal ?')) handleUpdateOutcome(sig._id, sig._outcome, 'skip'); }} title="Ignorer — sortie non comptabilisée"
                                style={{ padding: '4px 8px', background: 'none', border: '1px solid rgba(100,100,100,0.15)', borderRadius: '4px', color: '#3a3a3a', fontSize:'12px', fontFamily: 'inherit', cursor: 'pointer', transition: 'all 0.12s' }}
                          ...
                          {sig.context && <div style={{ marginTop: '6px', fontSize:'12px', color: '#5a6a82', fontStyle: 'italic' }}>{sig.context}</div>}
                        </div>
                      );
                    })}
                  </div>
                  <div style={{ marginTop: '8px', fontSize:'12px', color: '#3a1818', lineHeight: '1.6' }}>
              ...
              {completedSignals.length > 0 && (
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                    <span style={{ fontSize:'12px', color: '#5a6a82', letterSpacing: '2px', fontWeight: '700' }}>TERMINÉS</span>
                    <span style={{ fontSize:'12px', background: 'rgba(136,153,187,0.10)', border: '1px solid rgba(136,153,187,0.18)', color: '#3a9a5a', padding: '1px 8px', borderRadius: '10px', fontWeight: '700' }}>{completedSignals.length}</span>
                    {completedSignals.filter(s => s._outcome === 'win').length > 0 && (
                      <span style={{ fontSize:'12px', color: '#8899bb', fontWeight: '700' }}>✓ {completedSignals.filter(s => s._outcome === 'win').length}W</span>
                    )}
                    {completedSignals.filter(s => s._outcome === 'loss').length > 0 && (
                      <span style={{ fontSize:'12px', color: '#ff4455', fontWeight: '700' }}>✗ {completedSignals.filter(s => s._outcome === 'loss').length}L</span>
                    )}
                    {completedSignals.filter(s => s._outcome === 'be').length > 0 && (
                      <span style={{ fontSize:'12px', color: '#f0c020', fontWeight: '700' }}>— {completedSignals.filter(s => s._outcome === 'be').length}BE</span>
                    )}
                  </div>
                      ...
                      <thead>
                        <tr style={{ background: 'rgba(136,153,187,0.05)', borderBottom: '1px solid rgba(136,153,187,0.10)' }}>
                          {['HEURE', 'TF', 'DIR', 'ENTRY', 'SL', 'TP', 'R:R', 'CONTEXTE', 'RÉSULTAT', ''].map(h => (
                            <th key={h} style={{ padding: '8px 10px', textAlign: 'left', fontSize:'12px', color: '#5a6a82', letterSpacing: '1.5px', fontWeight: '700' }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                          ...
                          return (
                            ...
                            >
                              ...
                              <td style={{ padding: '8px 6px' }}>
                                {tf
                                  ? <span style={{ fontSize:'12px', color: '#3a9a5a', background: 'rgba(136,153,187,0.08)', border: '1px solid rgba(136,153,187,0.14)', padding: '1px 5px', borderRadius: '3px', fontWeight: '700' }}>{tf}</span>
                                  : <span style={{ fontSize:'12px', color: '#3a1818' }}>—</span>
                                }
                              </td>
                              ...
                              <td style={{ padding: '8px 10px', color: '#5868a0', fontSize:'12px', maxWidth: '120px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sig.context || '—'}</td>
                              <td style={{ padding: '6px 8px' }}>
                                <div style={{ display: 'flex', gap: '3px', alignItems: 'center' }}>
                                  {sig.type === 'test' && <span style={{ fontSize:'11px', background: 'rgba(240,120,32,0.15)', border: '1px solid rgba(240,120,32,0.4)', color: '#f07820', padding: '1px 4px', borderRadius: '2px', fontWeight: '700', letterSpacing: '1px', marginRight: '2px' }}>TEST</span>}
                                  {sig._autoOutcome && sig._outcome && (
                                    <span style={{ fontSize:'11px', background: 'rgba(0,170,255,0.15)', border: '1px solid rgba(0,170,255,0.3)', color: '#00aaff', padding: '1px 4px', borderRadius: '2px', fontWeight: '700', letterSpacing: '1px', marginRight: '2px' }}>AUTO</span>
                                  )}
                                  {[['win','W','#8899bb'],['loss','L','#ff4455'],['be','BE','#f0c020']].map(([key, label, c]) => {
                                    const sel = sig._outcome === key;
                                    return (
                                      <button key={key} onClick={() => handleUpdateOutcome(sig._id, sig._outcome, key)}
                                        style={{ padding: '2px 6px', borderRadius: '3px', border: `1px solid ${sel ? c + '80' : 'rgba(136,153,187,0.12)'}`, background: sel ? `${c}20` : 'transparent', color: sel ? c : '#5a6a82', fontSize:'12px', fontFamily: 'inherit', fontWeight: sel ? '700' : '400', cursor: 'pointer', transition: 'all 0.12s' }}
                              ...
                              <td style={{ padding: '6px 8px' }}>
                                <button onClick={() => handleSaveSignal(sig)}
                                  style={{ background: 'none', border: '1px solid rgba(136,153,187,0.18)', borderRadius: '3px', color: '#5a6a82', fontSize:'12px', fontFamily: 'inherit', cursor: 'pointer', padding: '2px 7px', transition: 'all 0.12s' }}
                                  onMouseEnter={e => { e.currentTarget.style.color = '#8899bb'; e.currentTarget.style.borderColor = 'rgba(136,153,187,0.45)'; }}
                                  onMouseLeave={e => { e.currentTarget.style.color = '#5a6a82'; e.currentTarget.style.borderColor = 'rgba(136,153,187,0.18)'; }}
                                >+</button>
                              </td>
      ...
      {tab === 'chart' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {/* Explanation */}
          <div style={{ padding: '14px 18px', background: 'rgba(0,170,255,0.05)', border: '1px solid rgba(0,170,255,0.15)', borderRadius: '6px', fontSize:'13px', color: '#5a8a9a', lineHeight: '1.7' }}>
            ...
            {[
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ fontSize:'13px', color: '#5a6a82' }}>{btn.sub}</div>
                </div>
          ...
          <div style={{ background: 'rgba(14,15,22,0.4)', border: '1px solid rgba(136,153,187,0.10)', borderRadius: '6px', padding: '14px 18px' }}>
            <div style={{ fontSize:'12px', color: '#5a6a82', letterSpacing: '2px', marginBottom: '10px' }}>OUVRIR UN SYMBOLE PERSONNALISÉ</div>
          ...
          {/* Tip */}
          <div style={{ padding: '10px 14px', background: 'rgba(240,192,32,0.04)', border: '1px solid rgba(240,192,32,0.12)', borderRadius: '5px', fontSize:'13px', color: '#6a5a20', lineHeight: '1.6' }}>
      ...
      {tab === 'setup' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ background: 'rgba(14,15,22,0.5)', border: '1px solid rgba(136,153,187,0.18)', borderRadius: '8px', padding: '16px 20px' }}>
            <div style={{ fontSize:'12px', color: '#8899bb', letterSpacing: '2px', marginBottom: '14px', fontWeight: '700' }}>🔍 DIAGNOSTIQUE RAPIDE</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '14px' }}>
              ...
              <div style={{ background: serverOk ? 'rgba(136,153,187,0.08)' : 'rgba(255,68,85,0.06)', border: `1px solid ${serverOk ? 'rgba(136,153,187,0.22)' : 'rgba(255,68,85,0.2)'}`, borderRadius: '6px', padding: '10px 12px' }}>
                <div style={{ fontSize:'12px', color: '#5a6a82', letterSpacing: '2px', marginBottom: '6px' }}>SERVEUR LOCAL</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  ...
                  <span style={{ fontSize:'13px', fontWeight: '700', color: serverOk ? '#8899bb' : '#ff4455' }}>{serverOk ? 'ACTIF' : 'ERREUR'}</span>
                </div>
                <div style={{ fontSize:'12px', color: '#5a6a82', marginTop: '4px' }}>Port {port}</div>
              </div>
              {/* Test local */}
              <div style={{ background: testStatus === 'ok' ? 'rgba(136,153,187,0.08)' : testStatus === 'err' ? 'rgba(255,68,85,0.06)' : 'rgba(14,15,22,0.3)', border: `1px solid ${testStatus === 'ok' ? 'rgba(136,153,187,0.22)' : testStatus === 'err' ? 'rgba(255,68,85,0.2)' : 'rgba(136,153,187,0.10)'}`, borderRadius: '6px', padding: '10px 12px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div style={{ fontSize:'12px', color: '#5a6a82', letterSpacing: '2px', marginBottom: '6px' }}>TEST LOCAL</div>
                {testStatus === 'ok' && <div style={{ fontSize:'13px', color: '#8899bb', fontWeight: '700' }}>✓ Signal reçu !</div>}
                {testStatus === 'err' && <div style={{ fontSize:'13px', color: '#ff4455', fontWeight: '700' }}>✗ Serveur KO</div>}
                {testStatus === null && <div style={{ fontSize:'12px', color: '#5a6a82' }}>Envoi direct localhost</div>}
                <button onClick={handleTestWebhook} style={{ marginTop: '6px', padding: '4px 10px', background: 'transparent', border: '1px solid rgba(136,153,187,0.35)', borderRadius: '4px', color: '#566880', fontSize:'12px', fontFamily: 'inherit', cursor: 'pointer', transition: 'all 0.15s' }}
                  onMouseEnter={e => e.currentTarget.style.background = 'rgba(136,153,187,0.10)'}
                  onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                >TESTER →</button>
              </div>
              ...
              <div style={{ background: 'rgba(14,15,22,0.3)', border: '1px solid rgba(136,153,187,0.10)', borderRadius: '6px', padding: '10px 12px' }}>
                <div style={{ fontSize:'12px', color: '#5a6a82', letterSpacing: '2px', marginBottom: '6px' }}>SIGNAUX REÇUS</div>
            ...
            <div style={{ fontSize:'13px', color: '#3a5a3a', lineHeight: '1.7', background: 'rgba(0,0,0,0.2)', padding: '10px 12px', borderRadius: '5px' }}>
          ...
          <div style={{ background: 'rgba(4,12,8,0.8)', border: '1px solid rgba(136,153,187,0.12)', borderRadius: '8px', padding: '14px 18px' }}>
            ...
            {webhookLogs.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '20px', fontSize:'13px', color: '#3a1818' }}>
                ...
                {webhookLogs.map((log, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '4px 8px', background: typeBg, borderRadius: '4px', borderLeft: `2px solid ${typeColor}` }}>
                      <span style={{ fontSize:'12px', color: '#5a6a82', minWidth: '50px', fontFamily: 'monospace' }}>{time}</span>
                      <span style={{ fontSize:'12px', background: `${typeColor}20`, color: typeColor, padding: '0px 5px', borderRadius: '2px', fontWeight: '700', minWidth: '45px', textAlign: 'center' }}>{log.status}</span>
                      <span style={{ fontSize:'12px', color: log.status === 400 ? '#ff7766' : '#7888a0', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{log.label}</span>
                      <span style={{ fontSize:'12px', color: '#3a1818', fontFamily: 'monospace' }}>{log.from?.replace('::ffff:', '') ?? ''}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{ fontSize:'13px', color: '#6878a0', lineHeight: '1.6', background: 'rgba(0,255,136,0.03)', border: '1px solid rgba(136,153,187,0.12)', borderRadius: '6px', padding: '16px 20px' }}>
            <div style={{ fontSize:'12px', color: '#8899bb', letterSpacing: '2px', marginBottom: '12px', fontWeight: '700' }}>COMMENT ÇA FONCTIONNE</div>
          ...
          {[
            {
              ...
              content: (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <div style={{ fontSize:'13px', color: '#6878a0' }}>Télécharge ngrok sur <span style={{ color: '#00aaff' }}>ngrok.com</span> (gratuit) puis lance :</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <code style={{ flex: 1, background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(136,153,187,0.12)', borderRadius: '4px', padding: '8px 12px', fontSize:'13px', color: '#8899bb', fontFamily: 'monospace' }}>
                      ngrok http {port}
                    </code>
                    <button onClick={() => copyText(`ngrok http ${port}`, 'ngrok')} style={{ padding: '7px 12px', background: 'transparent', border: '1px solid rgba(136,153,187,0.22)', borderRadius: '4px', color: '#5a6a82', fontSize:'12px', fontFamily: 'inherit', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                      {copied === 'ngrok' ? '✓' : 'COPIER'}
                    </button>
                  </div>
                  <div style={{ fontSize:'13px', color: '#5a6a82' }}>Tu obtiendras une URL du type : <span style={{ color: '#f0c020' }}>https://abc123.ngrok.io</span></div>
                </div>
              )
            },
            {
              ...
              content: (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize:'13px', color: '#6878a0', lineHeight: '1.7' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', background: 'rgba(255,68,85,0.05)', border: '1px solid rgba(255,68,85,0.15)', borderRadius: '5px', padding: '8px 12px' }}>
                    <div style={{ fontSize:'12px', color: '#ff6644', fontWeight: '700', letterSpacing: '1px' }}>⚠ RÉGLAGES CRITIQUES</div>
                    <div>• Condition : <code style={{ color: '#8899bb', background: 'rgba(136,153,187,0.12)', padding: '1px 5px', borderRadius: '3px', fontSize:'12px' }}>Alert() function calls only</code></div>
            ...
            },
            {
              ...
              content: (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div style={{ fontSize:'13px', color: '#6878a0' }}>Envoie un signal de test depuis le terminal :</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <code style={{ flex: 1, background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(136,153,187,0.12)', borderRadius: '4px', padding: '8px 12px', fontSize:'13px', color: '#dde4ef', fontFamily: 'monospace', lineHeight: '1.4' }}>
            ...
            <div key={step.n} style={{ display: 'flex', gap: '14px', background: 'rgba(14,15,22,0.4)', border: '1px solid rgba(136,153,187,0.10)', borderRadius: '6px', padding: '16px 18px' }}>
              <div style={{ width: '26px', height: '26px', borderRadius: '50%', background: 'rgba(136,153,187,0.14)', border: '1px solid rgba(136,153,187,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize:'13px', fontWeight: '700', color: '#8899bb', flexShrink: 0 }}>{step.n}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize:'13px', fontWeight: '700', color: '#dde4ef', marginBottom: '8px', letterSpacing: '1px' }}>{step.title}</div>
                {step.content}
              </div>
            </div>
          ))}
          ...
          <div style={{ background: 'rgba(14,15,22,0.4)', border: '1px solid rgba(136,153,187,0.10)', borderRadius: '6px', padding: '16px 18px' }}>
            <div style={{ fontSize:'12px', color: '#5a6a82', letterSpacing: '2px', marginBottom: '10px' }}>FORMAT JSON ACCEPTÉ PAR LE WEBHOOK</div>
            <pre style={{ margin: 0, fontSize:'13px', color: '#6878a0', fontFamily: 'monospace', lineHeight: '1.6', background: 'rgba(0,0,0,0.3)', padding: '12px', borderRadius: '4px', overflow: 'auto' }}>{`{
  "symbol":    "MNQ",          // requis
      ...
      {tab === 'pine' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            {PINE_BOTS.map(bot => (
              <button key={bot.id} onClick={() => setSelectedPine(bot.id)}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
                  <div style={{ fontSize:'13px', fontWeight: '700', color: selectedPine === bot.id ? bot.color : '#dde4ef' }}>{bot.name}</div>
                  <div style={{ fontSize:'12px', background: `${bot.color}18`, border: `1px solid ${bot.color}30`, color: bot.color, padding: '1px 5px', borderRadius: '3px', fontWeight: '700' }}>{bot.tf}</div>
                </div>
                <div style={{ fontSize:'12px', color: '#5a6a82' }}>SL ×{bot.sl} · TP ×{(bot.sl * 2).toFixed(1)} · R:R 1:2</div>
                <div style={{ fontSize:'12px', color: '#5a6a82' }}>HTF {bot.htfTf === 'D' ? '1J' : bot.htfTf + 'min'} · score ≥ {bot.minScore}/5</div>
              </button>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 16px', background: `${activePine.color}08`, border: `1px solid ${activePine.color}25`, borderRadius: '7px' }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize:'13px', color: '#6878a0', lineHeight: '1.7' }}>
                Bot ID : <code style={{ color: activePine.color, background: `${activePine.color}15`, padding: '1px 6px', borderRadius: '3px', fontSize:'13px' }}>{activePine.botId}</code>
              ...
              <div style={{ fontSize:'12px', color: '#3a5a3a', marginTop: '2px' }}>
            ...
            <button onClick={() => copyText(generateScript(activePine), `pine_${activePine.id}`)}
              style={{ padding: '8px 16px', background: copied === `pine_${activePine.id}` ? `${activePine.color}25` : 'transparent', border: `1px solid ${activePine.color}50`, borderRadius: '5px', color: activePine.color, fontSize:'13px', fontFamily: 'inherit', cursor: 'pointer', letterSpacing: '1px', transition: 'all 0.15s', whiteSpace: 'nowrap' }}>
          ...
          <div style={{ background: 'rgba(4,12,8,0.9)', border: `1px solid ${activePine.color}18`, borderRadius: '6px', overflow: 'auto', maxHeight: '480px' }}>
            <pre style={{ margin: 0, padding: '16px 18px', fontSize:'13px', color: '#7aaa8a', fontFamily: "'JetBrains Mono','Fira Code',monospace", lineHeight: '1.65', whiteSpace: 'pre' }}>
          ...
          <div style={{ padding: '12px 16px', background: 'rgba(0,170,255,0.04)', border: '1px solid rgba(0,170,255,0.12)', borderRadius: '6px', fontSize:'13px', color: '#3a5a6a', lineHeight: '1.8' }}>
            ...
            {PINE_BOTS.map(bot => (
              <div key={bot.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '2px' }}>
                <span style={{ fontSize:'12px', background: `${bot.color}18`, border: `1px solid ${bot.color}30`, color: bot.color, padding: '1px 5px', borderRadius: '2px', fontWeight: '700', minWidth: '36px', textAlign: 'center' }}>{bot.tf}</span>
                <span style={{ color: '#5868a0' }}>→ chart MNQ1! <strong style={{ color: '#7888a0' }}>{bot.tf}</strong> · indicateur <code style={{ color: bot.color, fontSize:'12px' }}>{bot.botId}</code> · alerte webhook</span>
              </div>
            ))}
            <div style={{ marginTop: '6px', color: '#2a5a4a', fontSize:'12px' }}>Tous les bots pointent vers le même webhook URL ngrok — les signaux arrivent séparés par bot ID.</div>
          </div>

          {/* ── PINE EDIT — Indicateur Analyse ── */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', margin: '4px 0' }}>
            <div style={{ height: '1px', flex: 1, background: 'rgba(240,120,32,0.2)' }} />
            <span style={{ fontSize:'12px', color: '#f07820', letterSpacing: '2px', fontWeight: '700', whiteSpace: 'nowrap' }}>🔬 INDICATEUR ANALYSE — ICT EDIT</span>
            <div style={{ height: '1px', flex: 1, background: 'rgba(240,120,32,0.2)' }} />
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '14px', padding: '14px 16px', background: 'rgba(240,120,32,0.05)', border: '1px solid rgba(240,120,32,0.2)', borderRadius: '7px' }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize:'13px', fontWeight: '700', color: '#f07820', marginBottom: '5px' }}>ICT Setup — Liquidity + MSS + FVG</div>
              <div style={{ fontSize:'13px', color: '#5a5a3a', lineHeight: '1.7' }}>
                Indicateur d'analyse visuelle qui envoie des signaux <span style={{ background: 'rgba(240,120,32,0.15)', border: '1px solid rgba(240,120,32,0.4)', color: '#f07820', padding: '0px 5px', borderRadius: '2px', fontSize:'12px', fontWeight: '700' }}>TEST</span> dans le dashboard.
              </div>
              <div style={{ fontSize:'12px', color: '#4a4a2a', lineHeight: '1.8', marginTop: '4px' }}>
                <div>→ Liquidity sweep (EQH/EQL) · MSS · FVG · Order Block</div>
                <div>→ Bot ID : <code style={{ color: '#f07820', fontSize:'12px' }}>ICT_EDIT</code> · R:R 1:2 · TP calculé dynamiquement</div>
                <div>→ BE auto si retour à l'entrée · BE manuel via settings Pine</div>
              </div>
              <div style={{ marginTop: '8px', background: 'rgba(255,68,85,0.06)', border: '1px solid rgba(255,68,85,0.2)', borderRadius: '5px', padding: '8px 12px', fontSize:'12px', color: '#7a4a3a', lineHeight: '1.7' }}>
            ...
            <button onClick={() => copyText(PINE_EDIT_SCRIPT, 'pine_edit')}
              style={{ padding: '8px 16px', background: copied === 'pine_edit' ? 'rgba(240,120,32,0.25)' : 'transparent', border: '1px solid rgba(240,120,32,0.5)', borderRadius: '5px', color: '#f07820', fontSize:'13px', fontFamily: 'inherit', cursor: 'pointer', letterSpacing: '1px', transition: 'all 0.15s', whiteSpace: 'nowrap', flexShrink: 0 }}>
