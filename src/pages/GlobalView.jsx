    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(160px,1fr))', gap: '8px', marginBottom: '12px' }}>
        {[
          ...
          { label: 'TRADES',    value: trades.length, color: '#dde4ef' },
...
// ── Calendrier ────────────────────────────────────────────────
function CalendarTab({ trades, setModal }) {
  return (
    <TradingCalendar
      trades={trades}
...
// ── Trades ────────────────────────────────────────────────────
function TradesTab({ trades, total }) {
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 'none' }}>
      ...
      {total === 0 ? (
        ...
        <>
          {/* Onglets */}
          <div style={{ display:'flex', gap:'4px', marginBottom:'20px', borderBottom:'1px solid rgba(136,153,187,0.10)' }}>
            {GLOBALVIEW_TABS.map(t => (
              <button key={t.key} onClick={() => setActiveTab(t.key)}
                style={{ padding:'8px 16px', background:'transparent', border:'none', borderBottom:`2px solid ${activeTab===t.key?'#8899bb':'transparent'}`, color:activeTab===t.key?'#8899bb':'#5a6a82', fontSize:'13px', fontFamily:'inherit', letterSpacing:'2px', fontWeight:'700', cursor:'pointer', transition:'all 0.15s', marginBottom:'-1px' }}>
                {t.label}
              </button>
            ))}
          </div>
          ...
          <div style={{ display: activeTab === 'overview' ? 'block' : 'none' }}>
            <OverviewTab
              pnl={pnl} fees={fees} winrate={winrate} wins={wins} losses={losses} pf={pf} total={total} accountsCount={accounts.length}
              dirStats={dirStats}
              bestDow={bestDow} worstDow={worstDow}
              bestSession={bestSession} worstSession={worstSession}
              bestHour={bestHour} worstHour={worstHour}
              bestPair={bestPair} worstPair={worstPair}
              bestEmotion={bestEmotion} worstEmotion={worstEmotion}
          ...
          <div style={{ display: activeTab === 'calendar' ? 'block' : 'none' }}>
            <CalendarTab trades={trades} setModal={setModal} />
          </div>

          {/* Trades */}
          <div style={{ display: activeTab === 'trades' ? 'block' : 'none' }}>
            <TradesTab trades={trades} total={total} />
          </div>
        </>
      )}
