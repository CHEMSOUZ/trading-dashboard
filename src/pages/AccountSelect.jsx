...
function CreateAccountModal({ onClose, onCreate }) {
  ...
  async function submit() {
    ...
    setSaving(true);
    const res = await window.accounts.create({ name: form.name.trim(), type: form.type, color: form.color, brokerAccountId: form.brokerAccountId.trim() });
    setSaving(false);
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#070d12', border: '1px solid rgba(0,255,136,0.2)', borderRadius: '10px', width: '100%', maxWidth: '480px', padding: '28px' }}>
        <div style={{ marginBottom: '24px' }}>
          <div style={{ fontSize: '11px', color: '#3a6a4a', letterSpacing: '3px', marginBottom: '6px' }}>NOUVEAU</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div>
            <div style={{ fontSize: '10px', color: '#3a6a4a', letterSpacing: '2px', marginBottom: '8px' }}>TYPE DE COMPTE</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {Object.entries(TYPE_LABELS).map(([key, info]) => (
                <div key={key} onClick={() => setForm(p => ({ ...p, type: key, color: info.color }))}
                  style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 14px', borderRadius: '6px', cursor: 'pointer', background: form.type === key ? `${info.color}12` : 'rgba(10,28,18,0.4)', border: `1px solid ${form.type === key ? info.color + '40' : 'rgba(0,255,136,0.08)'}`, transition: 'all 0.15s' }}>
                  <span style={{ fontSize: '18px' }}>{info.icon}</span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '13px', color: form.type === key ? info.color : '#c8d8c8', fontWeight: form.type === key ? '700' : '400' }}>{info.label}</div>
                    <div style={{ fontSize: '11px', color: '#3a6a4a' }}>{info.desc}</div>
                  </div>
                  <div style={{ width: '16px', height: '16px', borderRadius: '50%', border: `2px solid ${form.type === key ? info.color : '#2a5a3a'}`, background: form.type === key ? info.color : 'transparent', flexShrink: 0 }} />
                </div>
              ))}
            </div>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
                    <div style={{ width: '36px', height: '36px', borderRadius: '8px', background: `${acc.color}20`, border: `1px solid ${acc.color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px' }}>{typeInfo.icon}</div>
