...
const PHASE_OPTIONS = [
  { group: 'CHALLENGE — Compte Évaluation', phase: 'challenge', color: '#8899bb', options: [
...
function CTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{ background: 'rgba(8,9,16,0.97)', border: '1px solid rgba(136,153,187,0.22)', borderRadius: '4px', padding: '8px 12px', fontSize: '13px', fontFamily: 'inherit' }}>
      ...
      {payload.map((p, i) => (
        <div key={i} style={{ color: p.name === 'Floor' ? '#ff4455' : '#8899bb', fontWeight: '700' }}>
...
function MetricCard({ label, value, sub, color = '#dde4ef', alert = false }) {
  return (
    <div style={{ background: alert ? 'rgba(255,68,85,0.06)' : 'rgba(14,15,22,0.5)', border: `1px solid ${alert ? 'rgba(255,68,85,0.3)' : 'rgba(136,153,187,0.10)'}`, borderTop: `2px solid ${color}`, borderRadius: '6px', padding: '14px 16px' }}>
      <div style={{ fontSize: '12px', color: '#5a6a82', letterSpacing: '1.5px', marginBottom: '6px' }}>{label}</div>
      <div style={{ fontSize: '21px', fontWeight: '700', color, lineHeight: 1 }}>{value}</div>
      {sub && <div style={{ fontSize: '12px', color: alert ? '#ff8888' : '#5a6a82', marginTop: '5px' }}>{sub}</div>}
    </div>
  );
}
...
function DayDot({ date, pnl }) {
  const isWin = pnl > 0;
  return (
    <div title={`${date}: ${fmt(pnl, true)}`} style={{ width: '34px', height: '34px', borderRadius: '50%', background: isWin ? 'rgba(136,153,187,0.14)' : 'rgba(255,68,85,0.12)', border: `1.5px solid ${isWin ? '#8899bb' : '#ff4455'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', color: isWin ? '#8899bb' : '#ff4455', fontWeight: '700', cursor: 'default' }}>
      {isWin ? '✓' : '✗'}
    </div>
  );
}
...
function PhaseSelector({ account, onChanged }) {
  ...
  const groupColor = current?.color ?? '#5a6a82';
  ...
  return (
    <div style={{ position: 'relative' }}>
      <div style={{ background: `rgba(${groupColor === '#8899bb' ? '0,255,136' : '240,192,32'},0.04)`, border: `1px solid ${groupColor}20`, borderRadius: '6px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: '9px', color: '#5a6a82', letterSpacing: '2px', marginBottom: '3px' }}>PHASE DU COMPTE</div>
          {current ? (
            <div>
              <span style={{ fontSize: '14px', fontWeight: '700', color: '#e8edf8' }}>{current.phase === 'challenge' ? '🎯' : '💰'} {current.label} — {current.phase === 'challenge' ? 'Challenge' : 'Funded'}</span>
      ...
      {open && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 500, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={() => setOpen(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#0c0d16', border: '1px solid rgba(136,153,187,0.35)', borderRadius: '10px', padding: '24px', width: '560px', maxWidth: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <div>
                <div style={{ fontSize: '9px', color: '#5a6a82', letterSpacing: '2px', marginBottom: '4px' }}>PROPFIRM</div>
                <div style={{ fontSize: '16px', fontWeight: '700', color: '#e8edf8' }}>Sélectionner la phase du compte</div>
              </div>
              <button onClick={() => setOpen(false)} style={{ background: 'none', border: '1px solid #1e2c40', color: '#5868a0', width: '28px', height: '28px', borderRadius: '50%', cursor: 'pointer', fontSize: '14px' }}>×</button>
            </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '6px' }}>
                  {grp.options.map(opt => (
                    <div key={opt.value} onClick={() => setSelected(opt.value)}
                      style={{ padding: '10px 12px', borderRadius: '6px', cursor: 'pointer', background: selected === opt.value ? `rgba(${grp.color === '#8899bb' ? '0,255,136' : '240,192,32'},0.08)` : 'rgba(14,15,22,0.4)', border: `1px solid ${selected === opt.value ? grp.color + '44' : 'rgba(136,153,187,0.08)'}`, transition: 'all 0.15s' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '3px' }}>
                        ...
                        <span style={{ fontSize: '15px', fontWeight: '700', color: selected === opt.value ? '#e8edf8' : '#7888a0' }}>{opt.label}</span>
                      </div>
                      <div style={{ fontSize: '10px', color: '#5868a0', paddingLeft: '22px' }}>{opt.sub}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
              <button onClick={() => setOpen(false)} style={{ padding: '9px 18px', borderRadius: '5px', border: '1px solid #1e2c40', background: 'transparent', color: '#6878a0', fontSize: '12px', fontFamily: 'inherit', cursor: 'pointer' }}>Annuler</button>
              <button onClick={confirm} disabled={saving || !selected || selected === account?.type}
                style={{ padding: '9px 22px', borderRadius: '5px', background: !selected || selected === account?.type ? 'rgba(14,15,22,0.4)' : 'linear-gradient(135deg,rgba(136,153,187,0.22),rgba(0,170,85,0.1))', border: `1px solid ${!selected || selected === account?.type ? '#1e2c40' : 'rgba(136,153,187,0.40)'}`, color: !selected || selected === account?.type ? '#3c4c64' : '#8899bb', fontSize: '12px', fontFamily: 'inherit', fontWeight: '700', letterSpacing: '1px', cursor: saving || !selected || selected === account?.type ? 'not-allowed' : 'pointer' }}>
...
function ChallengeTab({ trades, manualBalance, setManualBalance, balanceInput, setBalanceInput, accountType }) {
  ...
  const status = accountLost ? { label: 'COMPTE PERDU ❌', color: '#ff4455' }
    : allPassed ? { label: '✅ CHALLENGE VALIDÉ !', color: '#8899bb' }
  ...
  const inp = { background: 'rgba(14,15,22,0.6)', border: '1px solid rgba(136,153,187,0.18)', borderRadius: '4px', padding: '7px 10px', color: '#dde4ef', fontSize: '13px', fontFamily: 'inherit', outline: 'none' };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      {canValidateDay2 && (
        <div style={{ background: 'rgba(136,153,187,0.10)', border: '1px solid rgba(136,153,187,0.35)', borderRadius: '6px', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <span style={{ fontSize: '20px' }}>⚡</span>
          <div>
            <div style={{ fontSize: '13px', fontWeight: '700', color: '#8899bb' }}>Validation J+2 possible !</div>
            <div style={{ fontSize: '12px', color: '#5868a0' }}>J1 dans les 50% ({fmt(day1Pnl, true)} ≤ {fmt(baseLimit)}) · Objectif atteint · Minimum 2 jours respecté</div>
          </div>
        </div>
      )}
      {day2Possible && (
      ...
      <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', gap: '6px' }}>
          ...
          <button onClick={saveBalance} style={{ background: 'rgba(136,153,187,0.12)', border: '1px solid rgba(136,153,187,0.28)', color: '#8899bb', padding: '7px 14px', borderRadius: '4px', fontSize: '13px', fontFamily: 'inherit', cursor: 'pointer' }}>MAJ</button>
        </div>
        <div style={{ background: `rgba(${status.color === '#ff4455' ? '255,68,85' : status.color === '#8899bb' ? '0,255,136' : '240,160,32'},0.08)`, border: `1px solid ${status.color}40`, borderRadius: '6px', padding: '8px 16px', fontSize: '13px', fontWeight: '700', color: status.color }}>{status.label}</div>
      </div>
      ...
      {targetAdjusted && (
        <div style={{ background: 'rgba(240,160,32,0.08)', border: '1px solid rgba(240,160,32,0.3)', borderRadius: '8px', padding: '14px 18px', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '13px', color: '#7888a0', lineHeight: '1.6', marginBottom: '8px' }}>
            ...
            <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
              {[
                ...
                { label: 'ANCIEN TARGET', value: `+${PROFIT_TARGET}$`, color: '#7888a0', strike: true },
                { label: 'NOUVEAU TARGET', value: `+${dynamicProfitTarget}$`, color: '#f0a020' },
              ].map(({ label, value, color, strike }) => (
                <div key={label} style={{ background: 'rgba(14,15,22,0.5)', borderRadius: '4px', padding: '8px 12px' }}>
                  <div style={{ fontSize: '12px', color: '#5a6a82', marginBottom: '3px' }}>{label}</div>
      ...
      {/* Rules */}
      <div style={{ background: 'rgba(14,15,22,0.4)', border: '1px solid rgba(136,153,187,0.10)', borderRadius: '8px', padding: '18px' }}>
        <div style={{ fontSize: '12px', color: '#5a6a82', letterSpacing: '2px', marginBottom: '14px' }}>🎯 RÈGLES CHALLENGE</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>

          {/* Drawdown fixe */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '13px', background: rule1 ? 'rgba(136,153,187,0.05)' : 'rgba(255,68,85,0.06)', border: `1px solid ${rule1 ? 'rgba(136,153,187,0.14)' : 'rgba(255,68,85,0.3)'}`, borderRadius: '6px' }}>
            <span style={{ fontSize: '19px' }}>{rule1 ? '✅' : '❌'}</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '14px', color: '#dde4ef', fontWeight: '600', marginBottom: '3px' }}>
                Max Drawdown — {MAX_LOSS.toLocaleString()}$
                <span style={{ fontSize: '11px', color: '#5a6a82', marginLeft: '8px' }}>Fixe — floor {floor.toLocaleString()}$</span>
              </div>
              <div style={{ fontSize: '13px', color: '#5868a0' }}>Balance minimum : {floor.toLocaleString()}$ · Marge : {distanceToFloor.toFixed(2)}$</div>
            </div>
            <div style={{ fontSize: '15px', fontWeight: '700', color: rule1 ? '#8899bb' : '#ff4455', flexShrink: 0 }}>{fmt(distanceToFloor)} de marge</div>
          </div>

          {/* Profit target */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '13px', background: rule2 ? 'rgba(136,153,187,0.05)' : 'rgba(14,15,22,0.4)', border: `1px solid ${rule2 ? 'rgba(136,153,187,0.14)' : 'rgba(136,153,187,0.08)'}`, borderRadius: '6px' }}>
            <span style={{ fontSize: '19px' }}>{rule2 ? '✅' : '⏳'}</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '14px', color: '#dde4ef', fontWeight: '600', marginBottom: '6px' }}>
              ...
              <ProgressBar label={null} current={Math.min(netProfit, dynamicProfitTarget)} max={dynamicProfitTarget} color={rule2 ? '#8899bb' : '#f0a020'} displayText={`${fmt(netProfit, true)} / +${dynamicProfitTarget}$`} />
            </div>
          </div>

          {/* Jours min */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '13px', background: rule3 ? 'rgba(136,153,187,0.05)' : 'rgba(14,15,22,0.4)', border: `1px solid ${rule3 ? 'rgba(136,153,187,0.14)' : 'rgba(136,153,187,0.08)'}`, borderRadius: '6px' }}>
            <span style={{ fontSize: '19px' }}>{rule3 ? '✅' : '⏳'}</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '14px', color: '#dde4ef', fontWeight: '600', marginBottom: '8px' }}>
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                ...
                {Array.from({ length: Math.max(0, MIN_DAYS - tradingDays) }).map((_, i) => (
                  <div key={i} style={{ width: '34px', height: '34px', borderRadius: '50%', border: '1.5px dashed #1e2c40', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', color: '#3a1818' }}>—</div>
                ))}
              </div>
            </div>
            <div style={{ fontSize: '15px', fontWeight: '700', color: rule3 ? '#8899bb' : '#f0a020', flexShrink: 0 }}>{tradingDays}/{MIN_DAYS}</div>
          </div>

          {/* Consistency */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '13px', background: 'rgba(14,15,22,0.3)', border: `1px solid ${isConsistencyBreached ? 'rgba(240,160,32,0.3)' : 'rgba(136,153,187,0.08)'}`, borderRadius: '6px' }}>
            <span style={{ fontSize: '19px' }}>{isConsistencyBreached ? '⚡' : bestDayNet > 0 ? '✅' : '⏳'}</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '14px', color: '#dde4ef', fontWeight: '600', marginBottom: '4px' }}>Règle 50% — Meilleur jour &lt; 50% du target</div>
              <div style={{ fontSize: '13px', color: '#5868a0', marginBottom: '8px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: '6px' }}>
                {[
                  { label: 'MEILLEUR JOUR', value: fmt(bestDayNet, true), color: isConsistencyBreached ? '#f0a020' : '#8899bb' },
                  { label: 'LIMITE (50%)', value: fmt(dynamicLimit), color: '#dde4ef' },
                  { label: day1Pnl !== null ? 'J1 P&L' : '—', value: day1Pnl !== null ? fmt(day1Pnl, true) : '—', color: day1Respected ? '#00aaff' : '#7888a0' },
                ].map(({ label, value, color }) => (
                  <div key={label} style={{ background: 'rgba(14,15,22,0.5)', borderRadius: '4px', padding: '8px', textAlign: 'center' }}>
                    <div style={{ fontSize: '12px', color: '#5a6a82', marginBottom: '3px' }}>{label}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(160px,1fr))', gap: '10px' }}>
        <MetricCard label="BALANCE NETTE" value={`${currentBalance.toFixed(2)}$`} color={rule2 ? '#8899bb' : '#dde4ef'} sub={manualBalance > 0 ? 'Manuelle' : 'Estimée'} />
        ...
        <MetricCard label="JOURS TRADÉS" value={`${tradingDays}`} color={rule3 ? '#8899bb' : '#f0a020'} sub={`Min: ${MIN_DAYS} · J1 ok: ${day1Respected ? 'oui ✓' : 'non'}`} />
      </div>

      {/* Charts */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
        <div style={{ background: 'rgba(14,15,22,0.4)', border: '1px solid rgba(136,153,187,0.10)', borderRadius: '6px', padding: '16px' }}>
          <div style={{ fontSize: '12px', color: '#5a6a82', letterSpacing: '2px', marginBottom: '12px' }}>PROGRESSION — Target +{dynamicProfitTarget}${targetAdjusted ? ' ⚡' : ''}</div>
          {equityWithTarget.length > 1 ? (
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={equityWithTarget} margin={{ top: 5, right: 5, bottom: 0, left: 5 }}>
                <defs><linearGradient id="cG" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#8899bb" stopOpacity={0.15}/><stop offset="95%" stopColor="#8899bb" stopOpacity={0}/></linearGradient></defs>
                <CartesianGrid stroke="rgba(136,153,187,0.05)" strokeDasharray="3 3" />
                <XAxis dataKey="date" tick={{ fill: '#5a6a82', fontSize: 11 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: '#5a6a82', fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={v => `${(v/1000).toFixed(0)}K`} domain={['auto','auto']} />
                <Tooltip content={<CTooltip />} />
                <ReferenceLine y={targetBalance} stroke={targetAdjusted ? '#f0a020' : '#8899bb'} strokeDasharray="4 4" strokeOpacity={0.6} label={{ value: `+${dynamicProfitTarget}$`, fill: targetAdjusted ? '#f0a020' : '#8899bb', fontSize: 11 }} />
                ...
                <Area type="monotone" dataKey="balance" name="Capital net" stroke="#8899bb" strokeWidth={2} fill="url(#cG)" dot={false} />
      ...
      {allPassed && (
        <div style={{ background: 'rgba(136,153,187,0.10)', border: '2px solid #8899bb', borderRadius: '8px', padding: '20px', textAlign: 'center', boxShadow: '0 0 30px rgba(136,153,187,0.12)' }}>
          <div style={{ fontSize: '32px', marginBottom: '8px' }}>🎉</div>
          <div style={{ fontSize: '19px', fontWeight: '700', color: '#8899bb', marginBottom: '6px' }}>CHALLENGE VALIDÉ !</div>
          <div style={{ fontSize: '13px', color: '#5868a0' }}>Toutes les règles sont respectées. Tu peux passer au compte Funded.</div>
        </div>
      )}
    </div>
  );
...
function FundedTab({ trades, manualBalance, setManualBalance, balanceInput, setBalanceInput, accountType }) {
  ...
  const status = accountLost ? { label: 'COMPTE PERDU', color: '#ff4455' }
    : aboveLock ? { label: '🔒 ZONE SÉCURISÉE', color: '#8899bb' }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ fontSize: '13px', color: '#5a6a82' }}>P&L net: <span style={{ color: pnlColor(totalNet), fontWeight: '700' }}>{fmt(totalNet, true)}</span></div>
      </div>

      {/* Drawdown section */}
      <div style={{ background: 'rgba(14,15,22,0.4)', border: `1px solid ${distanceToFloor < 500 ? 'rgba(255,68,85,0.3)' : 'rgba(136,153,187,0.10)'}`, borderRadius: '8px', padding: '18px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px', flexWrap: 'wrap' }}>
          <div style={{ fontSize: '12px', color: '#5a6a82', letterSpacing: '2px' }}>MAX DRAWDOWN — {MAX_LOSS.toLocaleString()}$</div>
          {isLocked
            ? <div style={{ background: 'rgba(136,153,187,0.12)', border: '1px solid rgba(136,153,187,0.35)', borderRadius: '4px', padding: '3px 8px', fontSize: '10px', color: '#8899bb', fontWeight: '700', letterSpacing: '1px' }}>🔒 FLOOR VERROUILLÉ À {ACCOUNT_SIZE.toLocaleString()}$</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(160px,1fr))', gap: '10px', marginBottom: '14px' }}>
          <MetricCard label="BALANCE" value={`${currentBalance.toFixed(2)}$`} color={aboveLock ? '#8899bb' : '#dde4ef'} sub={manualBalance > 0 ? 'Manuelle' : 'Estimée'} />
          ...
          <MetricCard label="MARGE" value={`${distanceToFloor.toFixed(2)}$`} color={distanceToFloor < 500 ? '#ff4455' : distanceToFloor < 1000 ? '#f0a020' : '#8899bb'} alert={distanceToFloor < 500} />
        </div>
        <div style={{ height: '20px', background: 'rgba(0,0,0,0.3)', borderRadius: '4px', overflow: 'hidden', position: 'relative' }}>
          <div style={{ height: '100%', width: `${Math.min((distanceToFloor / MAX_LOSS) * 100, 100)}%`, background: distanceToFloor < 500 ? 'linear-gradient(90deg,#ff4455,#ff6677)' : distanceToFloor < 1000 ? 'linear-gradient(90deg,#f0a020,#f0c040)' : 'linear-gradient(90deg,#566880,#8899bb)', borderRadius: '4px', transition: 'width 0.5s ease' }} />
        ...
        {isLocked && (
          <div style={{ marginTop: '10px', background: 'rgba(136,153,187,0.10)', border: '1px solid rgba(136,153,187,0.22)', borderRadius: '4px', padding: '8px 12px', fontSize: '13px', color: '#8899bb' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
        {[
          { key: 'A', title: '💸 PAYOUT STANDARD', desc: '5 jours gagnants nets minimum', ready: optionAReady,
            body: (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <ProgressBar label="Jours gagnants (net > 0)" current={Math.min(winDays,5)} max={5} color={winDays>=5?'#8899bb':'#f0a020'} displayText={`${winDays}/5 jours`} />
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                  {allDays.slice(-7).map(([date,pnl]) => <DayDot key={date} date={date} pnl={pnl}/>)}
                </div>
                <div style={{ fontSize: '13px', color: optionAReady?'#8899bb':'#5868a0' }}>
          ...
          { key: 'B', title: '⚡ PAYOUT CONSISTENCY', desc: '3 jours + règle 40% sur P&L net', ready: optionBReady,
            body: (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <ProgressBar label="Jours gagnants net (min 3)" current={Math.min(winDays,3)} max={3} color={winDays>=3?'#8899bb':'#f0a020'} displayText={`${winDays}/3 jours`} />
                <ProgressBar label="Meilleur jour / Total net positif" current={Math.min(bestDayPct,40)} max={40} color={bestDayPct<=40?'#8899bb':'#ff4455'} displayText={`${bestDayPct.toFixed(1)}%/40%`} />
                <div style={{ fontSize: '13px', color: optionBReady?'#8899bb':'#5868a0' }}>
          ...
          <div key={opt.key} onClick={() => { setLocalOption(opt.key); localStorage.setItem('pf_payout_option', opt.key); }}
            style={{ background: 'rgba(14,15,22,0.4)', border: `2px solid ${localOption===opt.key?'#8899bb':'rgba(136,153,187,0.10)'}`, borderRadius: '8px', padding: '16px', cursor: 'pointer', transition: 'all 0.2s' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div style={{ width: '17px', height: '17px', borderRadius: '50%', border: `2px solid ${localOption===opt.key?'#8899bb':'#3a1a1a'}`, background: localOption===opt.key?'#8899bb':'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  {localOption===opt.key && <div style={{ width: '7px', height: '7px', borderRadius: '50%', background: '#090a10' }} />}
                </div>
                <span style={{ fontSize: '12px', color: '#5a6a82', letterSpacing: '1.5px' }}>{opt.title}</span>
              </div>
              {opt.ready && localOption===opt.key && <span style={{ background: 'rgba(136,153,187,0.18)', border: '1px solid #8899bb', color: '#8899bb', fontSize: '12px', padding: '2px 8px', borderRadius: '3px', fontWeight: '700' }}>ÉLIGIBLE ✓</span>}
            </div>
            <div style={{ fontSize: '13px', color: '#5868a0', marginBottom: '12px' }}>{opt.desc}</div>
            {opt.body}
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
        <div style={{ background: 'rgba(14,15,22,0.4)', border: '1px solid rgba(136,153,187,0.10)', borderRadius: '6px', padding: '16px' }}>
          <div style={{ fontSize: '12px', color: '#5a6a82', letterSpacing: '2px', marginBottom: '12px' }}>COURBE CAPITAL + FLOOR</div>
          {equityPoints.length > 1 ? (
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={equityPoints} margin={{ top: 5, right: 5, bottom: 0, left: 5 }}>
                <defs><linearGradient id="fG" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#8899bb" stopOpacity={0.15}/><stop offset="95%" stopColor="#8899bb" stopOpacity={0}/></linearGradient></defs>
                <CartesianGrid stroke="rgba(136,153,187,0.05)" strokeDasharray="3 3" />
                <XAxis dataKey="date" tick={{ fill: '#5a6a82', fontSize: 11 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: '#5a6a82', fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={v => `${(v/1000).toFixed(0)}K`} domain={['auto','auto']} />
                <Tooltip content={<CTooltip />} />
                ...
                <Area type="monotone" dataKey="balance" name="Capital net" stroke="#8899bb" strokeWidth={2} fill="url(#fG)" dot={false} />
                <Area type="monotone" dataKey="floor" name="Floor" stroke="#ff4455" strokeWidth={1.5} fill="none" strokeDasharray="4 4" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          ) : <div style={{ height: '180px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#3a1818', fontSize: '13px' }}>Aucun trade</div>}
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 'none', fontFamily: "'JetBrains Mono','Fira Code',monospace" }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <div style={{ fontSize: '10px', color: '#5a6a82', letterSpacing: '3px', marginBottom: '4px' }}>PROPFIRM</div>
          <h1 style={{ fontSize: '23px', fontWeight: '700', color: '#e8edf8', margin: 0 }}>
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', marginBottom: '20px', background: 'rgba(14,15,22,0.5)', border: '1px solid rgba(136,153,187,0.12)', borderRadius: '8px', padding: '4px' }}>
        {[
          ...
          <button key={key} onClick={() => switchTab(key)}
            style={{ flex: 1, padding: '13px 16px', borderRadius: '6px', border: 'none', cursor: 'pointer', transition: 'all 0.2s', background: tab===key ? 'rgba(136,153,187,0.14)' : 'transparent', fontFamily: 'inherit' }}>
            <div style={{ fontSize: '14px', fontWeight: '700', color: tab===key ? '#8899bb' : '#6878a0', marginBottom: '2px' }}>{label}</div>
            <div style={{ fontSize: '12px', color: tab===key ? '#8a3a3a' : '#3a5a3a' }}>{desc}</div>
            {tab===key && <div style={{ height: '2px', background: '#8899bb', borderRadius: '2px', marginTop: '8px', boxShadow: '0 0 6px #8899bb' }} />}
          </button>
        ))}
      </div>
