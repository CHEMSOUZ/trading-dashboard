...
const NAV = [
  ...
  { to: '/profile',   label: 'Profil Trader', sub: 'Portrait IA · Calendrier psychologique',
    icon: <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="17" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><circle cx="12" cy="13.5" r="2.3"/><path d="M8 19c0-1.8 1.8-3 4-3s4 1.2 4 3"/></svg> },
  ...
  { to: '/budget',    label: 'Budget',        sub: 'Catégories · Allocation',
...
export default function Sidebar({ activeAccount, onSwitchAccount, onAccountUpdated, onManageAccounts, onToggleAiCoach, aiCoachOpen, onLogout, isDemoMode }) {
  ...
  return (
    <aside style={{ width: `${width}px`, flexShrink: 0, background: '#090a10', borderRight: '1px solid rgba(136,153,187,0.10)', display: 'flex', flexDirection: 'column', position: 'relative', userSelect: isDragging ? 'none' : 'auto' }}>
      ...
      {/* Demo mode banner */}
      {isDemoMode && (
        <div style={{ margin: '8px 10px 0', padding: '8px 10px', background: 'rgba(245,158,11,0.10)', border: '1px solid rgba(245,158,11,0.30)', borderRadius: '6px', fontSize: '10px', color: '#fbbf24', lineHeight: '1.4', textAlign: 'center', letterSpacing: '0.3px' }}>
          🔒 MODE DÉMO<br/>Abonnez-vous pour vos vraies données
        </div>
      )}
      ...
      <div style={{ padding: '0 10px 8px' }}>
        <button onClick={() => onLogout?.()}
          style={{ width: '100%', padding: '7px 0', background: 'transparent', border: '1px solid rgba(136,153,187,0.15)', borderRadius: '6px', color: '#5a6a82', fontSize: '11px', fontFamily: 'inherit', letterSpacing: '1px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', transition: 'all 0.15s' }}
          onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(255,51,68,0.35)'; e.currentTarget.style.color = '#ff6677'; }}
          onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(136,153,187,0.15)'; e.currentTarget.style.color = '#5a6a82'; }}
        >
